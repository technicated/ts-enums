...
function generateSpecFile(arity: number): void {
  if (arity < 0) {
    throw new Error('Bad arity < 0')
  }
  ...
  const template = `import test, { ExecutionContext } from 'ava'
import { Case, cases } from '../case'
${isGeneric ? `import { HKT${n} } from '../hkt'` : ''}
import { unit, Unit } from '../unit'
import { makeEnum${arity === 0 ? '' : arity} } from './make-enum'
...
`
  ...
  const fileName = `src/lib/subdir/make-enum-${arity}/make-enum.spec.ts`
  fs.writeFileSync(fileName, template)
}
