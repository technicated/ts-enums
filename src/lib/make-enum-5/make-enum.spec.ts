...
test('enum with proto', (t) => {
  class MyEnumProto<A, B, C, D, E> {
    prev(this: MyEnum<A, B, C, D, E>): MyEnum<A, B, C, D, E> {
      switch (this.case) {
        case 'empty':
          return MyEnum.empty()
        case 'a':
  ...
  const MyEnum = makeEnum5<MyEnumHKT>({ proto: MyEnumProto })
...
test('enum with type', (t) => {
  ...
  const MyEnum = makeEnum5<MyEnumHKT, MyEnumType>({ type: MyEnumType })
...
test('weird generics', (t) => {
  class MaybeProto<A, B, C, D, E> {
    map<V, W, X, Y, Z>(
      this: Maybe<A, B, C, D, E>,
      tx_a: (value: A) => V,
      tx_b: (value: B) => W,
      tx_c: (value: C) => X,
      tx_d: (value: D) => Y,
